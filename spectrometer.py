...
class App:
	...
	def __init__(self, args, window, window_title, video_source=0):
		...
		def select_points(event):
			# graph clicks and set the x vals in the labels
		...
		def clear_points():
			# reset the interface, not the calibration
		...
		#Slider for filter
		self.filt = tkinter.Scale(self.bottom_frame, from_=0, to=15, orient="horizontal", command=savfilter)
		...
		if self.vid.isOpened():
			...
			if ret:
				...
				if self.holdpeaks == False:
					...
					self.intensity = savitzky_golay(self.intensity,17,int(self.savpoly))
				...
				indexes = peakIndexes(self.intensity, thres=thresh/max(self.intensity), min_dist=self.mindist)
				for i in indexes:
...
App(arg_parser().parse_args(), tkinter.Tk(), "PySpectrometer V3.2")